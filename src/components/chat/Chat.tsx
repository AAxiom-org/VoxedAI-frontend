...
const ChatInterface = ({
  ...
  const fetchChatSessions = async () => {
    try {
      ...
      if (pendingSessionRef.current) {
        console.log(
          "Skipping session reset due to pending transition",
    ...
    try {
      ...
      if (isNoteOpen) {
        ...
        try {
          ...
          if (noteContent) {
            console.log(
              `Successfully fetched note content (${noteContent.length} characters)`,
            );
          } else {
  ...
  useEffect(() => {
    if (currentSessionId) {
      ...
      if (currentSessionId.startsWith("temp-")) {
        console.log(
          "Preserving UI state for temporary session:",
          currentSessionId,
        );
        return;
      }
  ...
  return (
    <div
      className={`flex flex-col ${simplified ? "h-full" : "h-screen"} w-full ${simplified ? "relative" : ""}`}
    >
      {!isInChat && !isInGrid ? (
        <HomeView
