...
const BlockNoteEditor = ({
  ...
  const checkForVirtualNote = useCallback(async () => {
    try {
      ...
      const { data: virtualNoteData, error: virtualNoteError } =
        await supabaseClient.storage.from("Vox").download(virtualPath);
  ...
  const handleAcceptChanges = useCallback(async () => {
    try {
      ...
      if (virtualPath) {
        const supabaseClient = await getSupabaseClient();
        await supabaseClient.storage.from("Vox").remove([virtualPath]);
      }
  ...
  const editor = React.useMemo(() => {
    try {
      ...
      try {
        ...
        console.error(
          "Critical error creating fallback editor:",
          fallbackError,
        );
    ...
    return (
      <div className="w-full h-full">
        ...
        <div className="p-4 bg-red-50 text-red-800 rounded-md">
          ...
          <p className="mb-4">
            We couldn't initialize the note editor. Please try refreshing the
            page.
          </p>
          <button
            onClick={onClose}
